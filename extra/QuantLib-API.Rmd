...
- Implement `Schedules` to generate date sequences based on calendars.
  - `cal_seq(start, stop, period, convention = "following", calendar = default_calendar())`
...
- Implement calendars.
  ...
  - `cal_holidays(start, stop, include_weekends = FALSE, calendar = default_calendar())`
  - Use a default calendar
    - `calendar = default_calendar()`
    - This should be overridable globally, would set into `calendar_env$calendar`
    - Defaults to `"united_states"`
  - Calendar object
    - `calendar()`
    - `new_calendar()`
  - `cal_adjust(x, convention = "following", calendar = default_calendar())`
  - `cal_advance(x, period, convention = "following", end_of_month = FALSE, calendar = default_calendar())`
    - `period` is a lubridate period object
  - `cal_count_business_days_between(start, stop, calendar = default_calendar())`
